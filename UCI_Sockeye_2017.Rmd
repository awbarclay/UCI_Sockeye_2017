...
###Check harvest dates sampled for each capture_location
```{r sample dates}
...
mix.sum.harvest<-right_join(mix.sum.prop,harvest_df[,c("rep_harvest","mixture_collection_f")],by="mixture_collection_f") %>% 
  group_by(mixture_collection_f,repunit_f) %>% 
  mutate(mean=mean*rep_harvest,median=median*rep_harvest,sd=sd*rep_harvest,lo5CI=lo5CI*rep_harvest,hi95CI=hi95CI*rep_harvest)

```
 
###Plot mixture estimates
####Barplot
```{r mixture estimate plots}

print(
mix.sum.harvest%>% 
  ggplot(aes(x=repunit_f, y = mean/1000, fill =repunit_f)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  geom_bar(stat="identity",position = "dodge") +
  geom_errorbar(aes(ymin = lo5CI/1000, ymax = hi95CI/1000, width = 0.3), position = "dodge")+
...
```
####Bubble plot
```{r unpresented harvests, hide=TRUE}

mix.sum.harvest_unrep<-harvest_df %>% 
  group_by(mixture_collection_f) %>% 
  summarise(mean=total_harvest-rep_harvest,rep_harvest=rep_harvest) %>% 
  mutate(repunit_f="Unrepresented",lo5CI=0,hi95CI=0,sd=0,median=0) %>% 
  full_join(mix.sum.harvest) %>% 
  mutate(repunit_f=factor(repunit_f,levels=c(groups,"Unrepresented")),mixture_collection_f=factor(mixture_collection_f,levels=c("Northern_Dist17","West_Kalgin17","Drift17","Drift_Corridor17","ESSN17"))) %>% 
...
stratified_annual<- repunit_trace %>% 
  ...
  full_join(harvest_df, by = c("mixture_collection" = "mixture_collection"))%>%
